...
$('#txtMessage').keyup(function (event) {
    if ($(this).val().trim().length > 0) {
        ...
        changeSendIconVisibility(false);
    }
});

function changeSendIconVisibility(val) {
    if (val) {
...
function joinChatRoom() {
    ...
    setCookie().then(function (val) {
        ...
        connection.invoke("joinChatRoom", _name, _username).catch(function (err) {
            return console.error(err.toString());
        });
    }).catch(function (err) {
...
// $(document).on('click', '.imgChat', function (e) {
//     e.preventDefault();
//     console.log('ljhchchchch');
